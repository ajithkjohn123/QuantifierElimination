...
#define	_CHELPER_H
    ...
    extern int GlobalAutonameCount;

    extern int inlining_ht_hash_hits;
    extern int inlining_ht_hash_misses; // number of hits and misses in the hash table for qualmode
    ...
    class CHelper
    {
	    ...
	    CDAGNode* Global_Inlining_Internal(CDAGNode*);
	     ...
	     CDAGNode* cloneWithRenameInternal(CDAGNode* parent, map<string, string> &RenameMap, map<CDAGNode*, CDAGNode*> &DPTable);
	     ...
	     CDAGNode* getOriginalCloneInternal(CDAGNode* parent, map<CDAGNode*, CDAGNode*> &LinearizationTable, map<CDAGNode*, CDAGNode*> &DPTable);
	     ...
	     // FNS FOR CHECKING EXACTNESS : 5/1/10
	      ...
	      CDAGNode* getSubtractionReplacedCloneInternal(CDAGNode* parent, map<CDAGNode*, CDAGNode*> &SubtractionTable, map<string, int> &WidthTable, map<CDAGNode*, CDAGNode*> &DPTable);
	      ...
	      string callgetDAGOfProperSkeltonFormatInYicesFormatWithWidthTableWithDP(CDAGNode *root, map<string, int> WidthTable, int &TempCount, FILE *fp, map<CDAGNode*, string> &LabelTable, set<string> &SupportOfUnrolling);
	      ...
	      string printInYicesFormatModified(CDAGNode *node, string context, map<CDAGNode*, int> &WidthTableForNodes, int Width_Required, map<CDAGNode*, string> &LabelTable, FILE *fp, map<string, int> &WidthTable, int &TempCount, map<CDAGNode*, string> &TypeTableOfITENodes, set<string> &SupportOfUnrolling);


	      /* Functions added on 14 June 2010 to print the transition relation in SMTLib format */
	      void printTRInFile(CDAGNode *TR, map<string, int> WidthTable);
	      void writeBVDeclarationsFromWidthTableInSMTLibFormatWithoutDeltaToggle(FILE *smt_file, map<string, int> WidthTable);


	      /* Functions added on 13 August 2010 to print the input to QuaLMoDe in SMTLib format */
	      void printQEliminatorResultsInFile(CDAGNode *original, map<string, int> WidthTable, list<string> VarsToElim);

	      string printInSMTLibFormatModified(CDAGNode *node, string context, map<CDAGNode*, int> &WidthTableForNodes, int Width_Required, map<CDAGNode*, string> &LabelTable, FILE *fp, map<string, int> &WidthTable, int &TempCount, map<CDAGNode*, string> &TypeTableOfITENodes, set<string> &SupportOfUnrolling);

	      void printDeltaTRInFile(CDAGNode *TR, map<string, int> &WidthTable);

              void printQEliminatorResultsInGivenFile(CDAGNode *original, map<string, int> WidthTable, list<string> VarsToElim, string SMTFileName);


	      /* Functions added on 1 Feruary 2011 onwards to add facility to simulate expressions */
	      bvatom lmdeEvaluator(CDAGNode *node, int width, map<string, string> &Variable_Value_Map, bool &boolean_result);
	      bool call_lmdeEvaluator(CDAGNode *node, map<string, string> &Variable_Value_Map, map<string, int> &MyWidthTable);
	      bool evaluate(CDAGNode *node, map<string, string> &Variable_Value_Map, map<string, int> &MyWidthTable);
	      bool evaluate_Internal(CDAGNode *node, map<string, string> &Variable_Value_Map, map<string, int> &MyWidthTable, map<CDAGNode*, bool> &DPTable);
	      string evaluateFunction(CDAGNode *node, map<string, string> &Variable_Value_Map, map<string, int> &MyWidthTable);
	      bvatom evaluateFunction_Internal(CDAGNode *node, map<string, string> &Variable_Value_Map, map<string, int> &MyWidthTable, map<CDAGNode*, bvatom> &DPTable);
	      bvatom call_lmdeEvaluatorReturningBV(CDAGNode *node, map<string, string> &Variable_Value_Map, map<string, int> &MyWidthTable);
	      bvatom lmdeEvaluatorModified(CDAGNode *node, int width, map<string, string> &Variable_Value_Map, bool &boolean_result, map<string, int> &MyWidthTable);

	      string ULINTToBinaryString(unsigned long long int i);
	      unsigned long long int getCoefOfChar(char c);
	      unsigned long long int findPower16(int power);
	      unsigned long long int hexStringToULINT(string x);

	      string findProblematicSignalsAtPreviousStateForSignal(CDAGNode *node, map<string, string> &Variable_Value_Map, map<string, int> &MyWidthTable, set<string> &ProblematicSignalsAtPreviousStateForSignal);
	      bvatom findProblematicSignalsAtPreviousStateForSignal_Internal(CDAGNode *node, map<string, string> &Variable_Value_Map, map<string, int> &MyWidthTable, set<string> &ProblematicSignalsAtPreviousStateForSignal, map<CDAGNode*, pair<bvatom, set<string> > > &DPTable);
	      bvatom call_lmdeEvaluatorReturningBVWithMinimumSupportFinding(CDAGNode *node, map<string, string> &Variable_Value_Map, map<string, int> &MyWidthTable, set<string> &ProblematicSignalsAtPreviousStateForSignal);
	      void findSmaller(set<string> &ReducedModelChild1, set<string> &ReducedModelChild2, set<string> &ReducedModel);
	      void printDAGInSMTLibFormatInGivenFile(CDAGNode *dag, map<string, int> &WidthTable, string SMTFileName);
	      void writeBVDeclarationsFromWidthTableInSMTLibFormatWithoutDeltaToggleForGivenSetOfVariables(FILE *smt_file, map<string, int> WidthTable, set<string> &MySet);
	      int getWidthOfLMDEWithSelects(CDAGNode *node, map<string, int> MyWidthTable);
	      ULINT findPowerWithLargerRange(ULINT number);

	      // ADDED ON 28-3-2011 FOR INTEGRATION WITH THE SSIMULATOR

	      void checkForPropSkeltonModified(CDAGNode* parent);
              void getAPsForDAGInPropSkelton(CDAGNode* parent, list<CDAGNode*> &APList);
              bool searchAPList(list<CDAGNode*> &APList, CDAGNode *target);
	      void copyList(list<CDAGNode*> &DestList, list<CDAGNode*> &SourceList);
              void showAPList(list<CDAGNode*> &APList);
	      void modifyAPListBasedOnVarsToElim(list<CDAGNode*> &APList, set<string> &VarsToElim);
	      CDAGNode* getLinearizedCloneWithGlobalCount(CDAGNode* parent, map<CDAGNode*, CDAGNode*> &LinearizationTable, map<string, int> &WidthTable, set<string> &VarsToElimOriginal, set<string> &VarsToElimRenamed);
	      string setTypesWithDP(CDAGNode *tree, string context, map<CDAGNode*, string> &TypeTableOfITENodes);
	      string setTypesWithDPInternal(CDAGNode *tree, string context, map<CDAGNode*, string> &TypeTableOfITENodes, HashTable<string, string> *PtrHTable);

	      // ADDED ON 28-3-2011 FOR INTEGRATION WITH THE SSIMULATOR ENDS HERE

	     // Added by Ajith John on 9 Oct 2013 starts here
		
	     // The following two functions help in converting dags like and(and(f1, f2), and(f2, f3))
	     // to and(f1, and(f2, f3)) 

	     CDAGNode* recreateAfterAndFlattening(CDAGNode* root);
	     void andFlattening(CDAGNode* root, set<CDAGNode*> &FlattenedChildren, set<CDAGNode*> &AlreadyFlattened);
	     ...
	     HashTable <string, CDAGNode*> Inlining_HT; // DP table for inlining
