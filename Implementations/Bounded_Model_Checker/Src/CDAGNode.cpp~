...
void CDAGNode :: getDAGOfProperSkeltonFormatInYicesFormatWithWidthTableToFileWithShift(string context, map<CDAGNode*, int> &WidthTableForNodes, int Width_Required, FILE *yices_fp, int shift)
{
  string dag_str;
	  ...
	  fprintf(yices_fp, "%s", dag_str.c_str());
		...
		if(name == ">")
			fprintf(yices_fp, "(bv-gt ");
		if(name == "<")
			fprintf(yices_fp, "(bv-lt ");
		if(name == ">=")
			fprintf(yices_fp, "(bv-ge ");
		if(name == "<=")
			fprintf(yices_fp, "(bv-le ");
		...
		//cout<<"\nEncountered constant with name = " << name << " and context = " << context << endl;    //debug code
		...
		if(context == "const")
			...
			// Added by Ajith on 24 Sep 2013
			else if(name[0] == '"') // binary
			  {
			    //cout << endl << name <<" : binary constant\n";
		
			    string dag_str = name;
			    dag_str.erase(dag_str.begin(), dag_str.begin()+1);
			    dag_str.erase(dag_str.end()-1, dag_str.end());

			    
			    // Note that Width_Required field in the case of binary constants is not set properly
			    dag_str = "0b" + dag_str;
			    			    
			    fprintf(yices_fp,"%s", dag_str.c_str());
			  }
			// Added by Ajith on 24 Sep 2013 ends here
			else // Decimal
			  {
			    //cout << endl << name <<" : decimal constant\n";
			...
			//else dag_str = name;
			...
			else // interpreting a Boolean constant other than true/false as a bit-vector
			  {
			   ...
			   // Added by Ajith on 3 Oct 2013 ends here
			    else // Decimal
			      {
				//dag_str = convertDecimalToBitvectorWithProperWidth(name, Width_Required);
...
string CDAGNode :: getDAGOfProperSkeltonFormatInNewSMTLibFormatWithWidthTable(string context, map<CDAGNode*, int> &WidthTableForNodes, int Width_Required)
{
		...
		// The only type of concat the function currently supports is the one with both the operands as variables 

		bool supported_concatenation = true;// This is kept true only if either each child is a variable or concat itself. i.e. supported_concatenation is kept true only if this node is of the form variable 1 @ variable 2 @ ...@ variable n

		bool constant_child_encountered = false;

		bool non_concat_child_encountered = false;
		...
		list<CDAGNode*>::iterator iout = out_list.begin();
		    ...
		    if(child->node_type && child->name != "&") // not a variable and not a concat
		      {
			supported_concatenation = false;

			if(child->node_type && child->Get_DAGnode_outlist_size()==0) // child is constant
			  {
			    constant_child_encountered = true;
			  }
			else
			  {
			    non_concat_child_encountered = true;
			  }
		...
		if(!supported_concatenation)
		  {
		    
		    if(constant_child_encountered) // We have this node like constant 1/variable 1 @ constant 2/variable 2 @ ... @ constant n/variable n. Hence while passing the width down, for constant pass its own width, and for variable we do not need to worry about width as width is not printed for variables
		      {
			cout<<"\nConstant child is encountered in function CDAGNode :: getDAGOfProperSkeltonFormatInNewSMTLibFormatWithWidthTable\n";

			dag_str = "(concat ";

			iout = out_list.begin();
			
			if((*iout)->node_type && (*iout)->Get_DAGnode_outlist_size()==0) // first child is constant
			  {
			    int child1_size = getSizeOfConstant((*iout)->name);
			    dag_str += (*iout)->getDAGOfProperSkeltonFormatInNewSMTLibFormatWithWidthTable("concat_bv", WidthTableForNodes, child1_size) + " ";
			    iout++;
			    dag_str += (*iout)->getDAGOfProperSkeltonFormatInNewSMTLibFormatWithWidthTable("bv", WidthTableForNodes, WidthTableForNodes[this] - child1_size) + " ";
				
			  }
			else // second child is constant
			  {
			    iout++;
			    
			    int child2_size = getSizeOfConstant((*iout)->name);

			    iout = out_list.begin();

			    dag_str += (*iout)->getDAGOfProperSkeltonFormatInNewSMTLibFormatWithWidthTable("bv", WidthTableForNodes, WidthTableForNodes[this] - child2_size) + " ";
			    iout++;

			    dag_str += (*iout)->getDAGOfProperSkeltonFormatInNewSMTLibFormatWithWidthTable("concat_bv", WidthTableForNodes, child2_size) + " ";
		    ...
		    else if(non_concat_child_encountered)
		      {
			cout<<"\nThis type of concat is not supported by function CDAGNode :: getDAGOfProperSkeltonFormatInNewSMTLibFormatWithWidthTable currently\n";
			exit(1);
		      }
		    else
		      {
			cout<<"\nYou are not supposed to reach here in CDAGNode :: getDAGOfProperSkeltonFormatInNewSMTLibFormatWithWidthTable\n";          
			exit(1);
		      }
		...
		if(context == "const")
			...
			// Added by Ajith on 24 Sep 2013
			else if(name[0] == '"') // binary
			  {
			    ...
			    // Note that Width_Required field in the case of binary constants
			    // is not set properly; but we know that Width_Required is dag_str.size()
			    char width_required_char[100];
			    sprintf(width_required_char, "%d", dag_str.size());
			    dag_str = "bv" + binaryStringToInteger(dag_str);
  			    dag_str += "[";
  			    dag_str += width_required_char;
			    dag_str += "] "; 
			  }
			// Added by Ajith on 24 Sep 2013 ends here
			else // Decimal
		...
		else if(context == "concat_bv")
		  {
			...
			dag_str = convertHexToBitvectorWithProperWidthInNewSMTLibFormat(name, Width_Required);
		      ...
		      {
			dag_str = convertBitStringToBitvectorWithProperWidthInNewSMTLibFormat(name, Width_Required);
		      }
		    return dag_str;
		  }
		else
...
string CDAGNode :: getSingleVariable()
{
    string dag_str = name;
    ...
    else
    {
        ...
        for(; iout != out_list.end(); iout++)
        {
            string dag_str_from_child = (*iout)->getSingleVariable();
...
int CDAGNode::getSizeOfConstant(string constant)
{

  if(constant[0] == 'x')// hex. constant
    {
      constant.erase(constant.begin(), constant.begin()+2);
      constant.erase(constant.end()-1, constant.end());
      // now hex is in proper hexa decimal form
      return 4*(constant.size());
    }
  else // binary constant
    {
      ...
      constant.erase(constant.begin(), constant.begin()+1);
      constant.erase(constant.end()-1, constant.end());
...
string CDAGNode::convertBitStringToBitvectorWithProperWidthInNewSMTLibFormat(string constant, int width)
{
  cout<<"\nconstant = "<<constant<<endl;
  cout<<"\nconstant = "<<constant<<endl;
  cout<<"\nconstant = "<<constant<<endl;
  ...
  if(constant.size() != width)
    {
      cout<<"\nWidth mismatch in CDAGNode::convertBitStringToBitvectorWithProperWidthInNewSMTLibFormat\n";
      exit(1);
    }
  ...
  cout<<"\nwidth_string = "<<width_string<<endl;
  cout<<"\nwidth_string = "<<width_string<<endl;
  cout<<"\nwidth_string = "<<width_string<<endl;
  cout<<"\nwidth_string = "<<width_string<<endl;
  cout<<"\nwidth_string = "<<width_string<<endl;
...
// Added by Ajith on 24 September 2013

string CDAGNode::binaryStringToInteger(string str)
{
  int length = str.size();
  ULINT num = 0;
  ULINT base = 1;
  for(int i = length-1;i >= 0;i--)
    {
      if((str[i]!='1') && (str[i]!='0'))
	{
	cout<<"Unable to convert binary string to integer: "<<str<<" in CDAGNode::binaryStringToInteger\n";
	exit(1);
	}
      if(str[i]=='1')
        {
  ...
  char num_char[100];  
  sprintf(num_char, "%llu", num);
  string num_string(num_char);
  return num_string;

}

// Added by Ajith on 24 September 2013 ends here
