  ...
  class HashTable
{
 ...
 public:
  ...
  HashTable(int size)
    {
      ...
      if(size<=0)
	{
	  *ht_log<<"Invalid value for size entered  "<<size<<endl;
	  *ht_log<<"Abnormal termination\n";
	  *ht_log<<"Exiting\n";
	  exit(0);
	}
  ...
  ~HashTable()
    {
      //Release the resources allocated to the HashTable nodes
      *ht_log<<"Deleting the resources allocated to the hashtable\n";
      int counter = 0;
      for(int i=0;i<SIZE;i++)
	{
	  hashTableIndexNode head = Buckets[i];
	  if(head.next == NULL)
	    {
	      continue;
	    }
	    ...
	    {
	      hashTableNode *current_node = head.next;
	      while(current_node!= NULL)
		{
		  hashTableNode *node_to_delete = current_node;
      ...
      *ht_log<<"Deleted "<<counter<<" number of nodes \n";
      ht_log->close();
    }
 ...
 public:
  ...
  HTStatusValue<Value> hashtable_insert(Key key, Value val)
    {
	  ...
	  *ht_log<<"HashTable Insertion Process\n";
	  *ht_log<<"Attempt to insert existing value in the hashtable with key "<<key<<endl;
      ...
      if(insert_result==false)
        {
          Value Dummy_Val;
	  *ht_log<<"HashTable Insertion Process\n";
	  *ht_log<<"Could not add created node in the collision list of nodes "<<key<<endl;
  ...
  HTStatusValue<Value> hashtable_delete(Key key)
    {
      ...
      if(head.next == NULL)
        {
          Value DummyValue;
	  *ht_log<<"HashTable Deletion Process\n";
	  *ht_log<<"Attempt to delete non-existing value in the hashtable "<<endl;
      ...
      Value DummyValue;
      *ht_log<<"HashTable Deletion Process\n";
      *ht_log<<"Could not find the node in the collision chain with key "<<key<<endl;
  ...
  HTStatusValue<Value> hashtable_search(Key key)
	...
	cerr<<"Node is null\n";	
          ...
          Value Dummy_Val;
	  *ht_log<<"HashTable Search Process\n";
	  *ht_log<<"Key Not found in the collision chain!! Key: "<<key<<endl;
